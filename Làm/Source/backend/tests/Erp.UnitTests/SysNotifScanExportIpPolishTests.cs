...
public sealed class SysNotifScanExportIpPolishTests
{
    private static (AppDbContext db, SysNotifScanExportIpService svc, Guid tenantId) Create(string name)
    {
        var opts = new DbContextOptionsBuilder<AppDbContext>().UseInMemoryDatabase(name).Options;
        var db = new AppDbContext(opts);
        return (db, new SysNotifScanExportIpService(db), Guid.NewGuid());
    }
    ...
    public void Prefs_SecurityEvent_BypassesMute()
    {
        ...
        Assert.True(SysNotifScanExportIpService.ShouldDeliverInAppStatic(prefs, "security.account_locked", DateTimeOffset.UtcNow));
        Assert.False(SysNotifScanExportIpService.ShouldDeliverInAppStatic(prefs, "wf.task.assigned", DateTimeOffset.UtcNow));
    }
    ...
    public void Prefs_QuietHours_BlocksNormalEvents()
    {
        ...
        Assert.False(SysNotifScanExportIpService.ShouldDeliverInAppStatic(prefs, "wf.task.assigned", night));
        Assert.True(SysNotifScanExportIpService.ShouldDeliverInAppStatic(prefs, "wf.task.assigned", day));
    }
    ...
    {
        Assert.True(SysNotifScanExportIpService.IpMatches("10.5.6.7", "10.0.0.0/8"));
        Assert.False(SysNotifScanExportIpService.IpMatches("11.0.0.1", "10.0.0.0/8"));
        Assert.True(SysNotifScanExportIpService.IpMatches("192.168.1.1", "192.168.1.1"));
    }
}
