...
/// <summary>
/// UC_SYS_009 (SSO), UC_SYS_031 (field ACL), UC_SYS_058 (config versions), UC_SYS_062 (push).
/// </summary>
public sealed class SysSsoFieldConfigPushService : ISysSsoFieldConfigPushService
{
    ...
    private readonly ISysNotifScanExportIpService _notifScanExportIp;
    private readonly IConfiguration _config;

    public SysSsoFieldConfigPushService(
        AppDbContext db,
        IJwtTokenService jwt,
        IDataScopeService scope,
        ISysPlatformService platform,
        ISysNotifScanExportIpService notifScanExportIp,
        IConfiguration config)
    {
        ...
        _scope = scope;
        _platform = platform;
        _notifScanExportIp = notifScanExportIp;
        _config = config;
    }
        ...
        Guid tenantId, SysSsoCallbackRequest req, string? ip, string? ua, CancellationToken ct = default)
    {
        ...
        await _notifScanExportIp.EnsureIpAllowedAsync(tenantId, ip, ct);
